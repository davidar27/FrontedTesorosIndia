...
interface EditMemberFormProps {
    member: TeamMember;
    onSave: (memberData: EditMemberData) => Promise<void>;
    onCancel: () => void;
    isLoading?: boolean;
}
...
const EditMemberForm: React.FC<EditMemberFormProps> = ({
    ...
    const [errors, setErrors] = useState<Partial<EditMemberData>>({});
    const [currentImage, setCurrentImage] = useState<string>(member.image);
    const [dragActive, setDragActive] = useState(false);

    const validateForm = (): boolean => {
        const newErrors: Partial<EditMemberData> = {};
    ...
    const handleImageChange = async (file: File) => {
        if (file) {
            try {
                ...
                setCurrentImage(URL.createObjectURL(webpFile));
            } catch (error) {
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                ...
                <div className="bg-gradient-to-r from-primary via-primary/80 to-primary/60 p-8 text-white">
                    <div className="flex justify-between items-center">
                        <div>
                            <h1 className="text-3xl font-bold mb-2">Actualizar Integrante</h1>
                            <p className="text-gray-100 text-lg">Actualiza la información de {member.name}</p>
                        </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-8">
                    ...
                    <div className="flex flex-col items-center">
                        <div className="relative mb-6">
                            <div
                                className={`w-32 h-32 rounded-full border-4 border-dashed transition-all duration-300 flex items-center justify-center cursor-pointer overflow-hidden ${dragActive
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex justify-between items-center">
                            <p className="text-xs text-gray-500">
                                Opcional: Actualiza la descripción para destacar las fortalezas del integrante
                            </p>
