...
const HeaderActions = ({ isEditMode = false, onToggleEditMode, onStatusChangeRequest, currentStatus }: HeaderActionsProps) => {
            ...
            >
                ...
                {canEditExperience && (
                    <div className="flex items-center gap-3">
                        ...
                        <div
                            className={`text-gray-700 px-3 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 ${isPublished ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}
                        >
                        ...
                        {isEditMode && (
                            <div className="relative group">
                                ...
                                <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                                    <div className="py-2">
                                        {permissions.canDelete && onStatusChangeRequest && (
                                            <button
                                                className={`w-full flex items-center gap-3 px-4 py-2 cursor-pointer scale-105
                                                    transition-all duration-300 text-sm ${isPublished ? 'text-red-500' : 'text-green-500'}`}
                                            ...
                                            >
                                                ...
                                                <span>{isPublished ? 'Desactivar Experiencia' : 'Activar Experiencia'}</span>
                                            </button>
                                        )}
                                    </div>
                                </div>
